        ...
        while($hasNextPage){
            ...
            if ($response->successful()){
                ...
                foreach($responseBody->data as $data){
                    Tag::upsert([
                        'tag_id'=>$data->tag_id,
                        'localization_name'=>json_encode($data->localization_name),
                        'localization_description'=>json_encode($data->localization_description)
                    ], ['tag_id']);
                }
                if (isset($responseBody->pagination, $responseBody->pagination->cursor)) {
    ...
    public function fetchUserStreams()
    {
        ...
        while($hasNextPage) {
            ...
            if ($response->successful()) {
                ...
                if (isset($responseBody->pagination, $responseBody->pagination->cursor) && $iterations <= 10) {
                    $cursor = $responseBody->pagination->cursor;
                }
                else if ($iterations>=10) {
                    $hasNextPage = false;
